...
pub fn create_installer(bcp47code: &str, version: &str, build: u64, zhfst_file: &Path, output_dir: &Path) {
	let package = "no.divvun.MacDivvun";
    let bundle_name = format!("{}.{}.bundle", package, bcp47code);

	create_bundle(&bundle_name, package, bcp47code, version, build, zhfst_file, output_dir);
	create_installer_from_bundle(APP_NAME, &output_dir.join(bundle_name), version, package, output_dir);
}

pub fn create_bundle(bundle_name: &str, package: &str, bcp47code: &str, version: &str, build: u64, zhfst_file: &Path, output_dir: &Path) {
    ...
    plist_file.write_all(make_plist(bcp47code, version, build, APP_NAME, package).as_bytes()).expect("plist to be written");
}
...
fn create_installer_from_bundle(app_name: &str, bundle_dir: &Path, version: &str, package: &str, output_dir: &Path) {
	...
	{
		let mut distribution_file = File::create(&distribution_path).expect("distribution file to be created");
		distribution_file.write_all(make_distribution(&app_name, package, &component_pkg_name).as_bytes()).expect("distribution file to be written");
	}
...
fn create_component_package(output_dir: &Path, bundle_dir: &Path, package: &str, version: &str) -> String {
	...
	let result = Command::new("pkgbuild")
		...
		.arg("/Library/Keyboard Layouts") // TODO: where
		.arg("--version")
		.arg(version)
		.arg(&pkg_name)
		.status()
